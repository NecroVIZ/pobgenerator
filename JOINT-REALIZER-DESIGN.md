...
2. **Проблема ModPool в Expert режиме:**
   ...
   * *Решение в дизайне:* Введение режима `tree_only`, при котором шмот фиксируется на эталонном экспертном, а оптимизируется исключительно дерево пассивных умений (шаг оптимизации снаряжения полностью пропускается).
